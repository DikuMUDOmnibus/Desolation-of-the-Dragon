...
/*static char rcsid[] = "$Id: id.c,v 1.8 2004/04/06 22:00:09 dotd Exp $";*/
...
void  start_auth( struct descriptor_data *d )
{
   ...
   if ( d->auth_fd < 0 && err == EAGAIN )
     bug( "Can't allocate fd for authorization check" );
   nonblock( d->auth_fd );
   ...
   *system = *ruser = '\0';
    ...
    */
    ...
    if ( ( len = read( d->auth_fd, d->abuf + d->auth_inc, \
     			(signed int)sizeof( d->abuf ) - 1 - d->auth_inc ) ) >= 0 )
    {
	d->auth_inc += len;
	d->abuf[d->auth_inc] = '\0';
    }
	...
	*t = '\0';
	...
	sprintf( log_buf, "auth reply ok, incoming user: [%s]", ruser );
        log_string_plus( log_buf, LOG_COMM, LEVEL_GOD, SEV_INFO );
/*      STRFREE(d->user);
