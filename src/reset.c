...
/*static char rcsid[] = "$Id: reset.c,v 1.24 2004/04/06 22:00:11 dotd Exp $";*/
                    ...
                    AREA_DATA *pArea )
{
    ...
    case 'H':
        ...
        for ( reset = pReset->prev; reset; reset = reset->prev )
            if ( (reset->command == 'O' || reset->command == 'P' ||
                 ...
                 (!pr || pr == reset->arg1) && obj_exists_index(reset->arg1) )
                break;
    ...
    case 'B':
        ...
        case BIT_RESET_MOBILE:
            for ( reset = pReset->prev; reset; reset = reset->prev )
                if ( reset->command == 'M' && mob_exists_index(reset->arg1) )
                    break;
            if ( reset && is_room_reset(reset, aRoom, pArea) )
                return TRUE;
            return FALSE;
                 ...
                 ROOM_INDEX_DATA *aRoom )
{
    ...
    if ( !str_prefix( arg, "mobile" ) )
    {
        ...
        if ( !(pRoom = find_room(ch, argument, aRoom)) )
            return;
        pReset = make_reset('M', 0, pMob->ivnum, num, pRoom->vnum);
    ...
    if ( !str_prefix(arg, "object") )
    {
        ...
        if ( !str_prefix( arg, "put" ) )
        {
            ...
            if ( (vnum = atoi(arg)) < 1 )
                vnum = 1;
            pReset = make_reset('P', reset->extra+1, pObj->ivnum, 0, vnum);
        ...
        if ( !str_prefix( arg, "give" ) )
        {
            ...
            if ( (vnum = atoi(arg)) < 1 )
                vnum = 1;
            pReset = make_reset('G', 1, pObj->ivnum, vnum, 0);
            INSERT(pReset, reset, pArea->last_reset, prev, next);
        ...
        if ( !str_prefix( arg, "equip" ) )
        {
            ...
            if ( (vnum = atoi(arg)) < 1 )
                vnum = 1;
            pReset = make_reset('E', 1, pObj->ivnum, vnum, num);
             ...
             is_number(arg) )
        {
            ...
            if ( (vnum = atoi(arg)) < 1 )
                vnum = 1;
            pReset = make_reset('O', 0, pObj->ivnum, vnum, pRoom->vnum);
...
void add_obj_reset( AREA_DATA *pArea, char cm, OBJ_DATA *obj, int v2, int v3 )
{
    ...
    if ( (cm == 'O' || cm == 'P') && obj->vnum == OBJ_VNUM_TRAP )
    {
        if ( cm == 'O' )
            add_reset(pArea, 'T', obj->value[3], obj->value[1], obj->value[0], v3);
        return;
    }
    add_reset( pArea, cm, (cm == 'P' ? iNest : 1), obj->vnum,
               v2, v3 );
    ...
    for ( inobj = obj->first_content; inobj; inobj = inobj->next_content )
        if ( inobj->vnum == OBJ_VNUM_TRAP )
            add_obj_reset(pArea, 'O', inobj, 0, 0);
    if ( cm == 'P' )
        iNest++;
    for ( inobj = obj->first_content; inobj; inobj = inobj->next_content )
        add_obj_reset( pArea, 'P', inobj, 1, obj->vnum );
    if ( cm == 'P' )
        iNest--;
    return;
}
...
void instaroom( AREA_DATA *pArea, ROOM_INDEX_DATA *pRoom, bool dodoors )
{
    ...
    for ( rch = pRoom->first_person; rch; rch = rch->next_in_room )
    {
        if ( !IS_NPC(rch) )
            continue;
        add_reset( pArea, 'M', 1, rch->vnum, rch->pIndexData->count,
                   pRoom->vnum );
...
/*
 ...
 if ( pObjIndex->rent < MIN_OBJ_RENT )
 return 0;
...
void reset_area( AREA_DATA *pArea )
{
    ...
    for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
    {
        ...
        case 'M':
            ...
            {
                bug( "Reset_area: 'M': bad mob vnum %d.", pReset->arg1 );
                continue;
            }
            ...
            mob = create_mobile(pMobIndex->ivnum);
            {
            ...
            mprog_birth_trigger(NULL, mob);
            economize_mobgold(mob);
            level = URANGE(0, GetMaxLevel(mob) - 2, LEVEL_AVATAR);
            break;
        ...
        case 'E':
            ...
            /*if ( pObjIndex->count > pReset->arg2 )*/
            if ( count_objs_for_reset(pObjIndex) > pReset->arg2 )
            {
                obj = NULL;
                lastobj = NULL;
                log_printf_plus(LOG_DEBUG, LEVEL_IMMORTAL, SEV_TREET, "reset_area: obj count %d/%d exceeds reset %d vnum %d",
                                pObjIndex->count,
                                count_objs_for_reset(pObjIndex),
                                pReset->arg2,
                                pReset->arg1 );
                break;
            }
            ...
            else if ( mob->pIndexData->pShop ) /* don't set inventory on equip */
                SET_BIT(obj->extra_flags, ITEM_INVENTORY);

            if (!obj->carried_by)
                bug( "Reset_area: '%c': obj %d not carried",
        ...
        case 'O':
             ...
             */
            obj = create_object(pObjIndex->ivnum);
            obj = obj_to_room(obj, pRoomIndex);
            if (!obj->in_room)
                bug( "Reset_area: 'O': obj %d not in room %d", pReset->arg1, pReset->arg3);
            lastobj = obj;
            break;
        ...
        case 'P':
            if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
            {
                bug( "Reset_area: 'P': bad obj vnum %d.", pReset->arg1 );
                continue;
            }
            if ( pReset->arg3 > 0 )
            {
            ...
            obj = create_object(pObjIndex->ivnum);
            obj = obj_to_obj(obj, to_obj);
            if (!obj->in_obj)
                bug( "Reset_area: 'P': obj %d not in obj %d", pReset->arg1, pReset->arg3);
            break;
        ...
        case 'T':
            if ( IS_SET(pReset->extra, TRAP_OBJ) )
            {
                ...
                pobj = make_trap( pReset->arg2, pReset->arg1, 10, pReset->extra );
            ...
            else
            {
                if ( !(pRoomIndex = get_room_index(pReset->arg3)) )
                {
                    bug( "Reset_area: 'T': bad room %d.", pReset->arg3 );
                    continue;
                }
                if ( pArea->nplayer > 0 ||
        ...
        case 'D':
            if ( !(pRoomIndex = get_room_index(pReset->arg1)) )
            {
                bug( "Reset_area: 'D': bad room vnum %d.", pReset->arg1 );
                continue;
            }
    ...
    for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
    {
        ...
        case 'H':
            ...
            sprintf(pbuf, "Hide %s (%d)\n\r", rname,
                    (pReset->arg1 > 0 ? pReset->arg1 : obj ? obj->ivnum : 0));
            break;
        case 'B':
            {
                ...
                case BIT_RESET_DOOR:
                    {
                        ...
                        door = (pReset->arg2 & BIT_RESET_DOOR_MASK)
                            >> BIT_RESET_DOOR_THRESHOLD;
                        door = URANGE(0, door, MAX_REXITS-1);
                ...
                case BIT_RESET_MOBILE:
                    ...
                    sprintf(pbuf, "Mobile %s (%d)", rname,
                            (pReset->arg1 > 0 ? pReset->arg1 : mob ? mob->ivnum : 0));
                    flagarray = a_flags;
                    break;
                default:
        ...
        case 'D':
            {
                ...
                pReset->arg2 = URANGE(0, pReset->arg2, MAX_REXITS-1);
                if ( !(room = get_room_index(pReset->arg1)) )
                    rname = "Room: *BAD VNUM*";
                else
                    rname = room->name;
