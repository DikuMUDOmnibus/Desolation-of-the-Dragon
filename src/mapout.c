...
/*static char rcsid[] = "$Id: mapout.c,v 1.7 2004/04/06 22:00:10 dotd Exp $";*/
...
add_new_room_to_map(CD *ch,MID *map,int row,int col,int proto_room,char code) {
	...
	if (!room_exists_index (i)) {
	    location = make_room (i);
	    if (!location) {
		bug ("next_rooms_avail: make_room failed");
		return - 1;
	    }
	    /* 
...
void map_to_rooms (CD * ch, MID * m_index) {
    ...
    if (!ch -> pnote) {
	bug ("map_to_rooms: ch->pnote==NULL!");
	return;
    }
    n = 0;
    row = col = 0;
