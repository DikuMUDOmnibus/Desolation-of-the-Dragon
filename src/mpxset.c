...
/*static char rcsid[] = "$Id: mpxset.c,v 1.8 2003/12/06 01:14:36 dotd Exp $";*/
...
void do_mpmset( CHAR_DATA *ch, char *argument )
{
        ...
        if( IS_NPC(victim) )   /* Broken by Haus... fixed by Thoric */
	{
          if ( value >= MAX_NPC_CLASS || value < 0 )
	  {
		progbug("MpMset: Invalid npc class", ch);
          ...
          victim->classes[value] = STAT_ACTCLASS;
	  return;
	}
	...
	victim->levels[FirstActive(victim)] = value;
	return;
    }
	...
	if ( !IS_NPC(victim) )
	{
	    progbug("MpMset: can't set pc max_mana", ch);
	    return;
	}
	...
	if ( !IS_NPC(victim) )
	{
	    progbug("MpMset: can't set pc max_move", ch);
	    return;
	}
...
*/
	...
	sprintf( log_buf, "%s raising glory of %s by %d ...",
	  ch->name, victim->name, value );
        log_string_plus( log_buf, LOG_MONITOR, LEVEL_LOG_CSET, SEV_INFO );
	...
	if ( ( victim->spec_fun = m_spec_lookup( arg3 ) ) == 0 )
	{
	...
	while ( argument[0] != '\0' )
	{
	   ...
	   else
	   {
	     ...
	     else if ( 1 << value == ACT_POLYMORPHED )
