...
/*static char rcsid[] = "$Id: grub.c,v 1.5 2003/01/26 23:43:54 dotd Exp $";*/
...
enum gr_field_type          /* enumerates the fields in the input record */
   {name, sex, ch_class, race, level, room, gold, clan, council,
    site, last, pkill};
                 ...
                 GO_STRUCT **p)
{
  ...
  if ( !d2_sw )
   ...
   for ( cou=0; cou<lim; cou++)
   {
      ...
      if ( !d2_sw )
         pager_printf(ch,
            "%-12s%3d %5d %-12s %2s %2s%3d%3d%3d%4d%4d%4d"
            "%2d%2d%2d%2d%2d%2d\n\r", 
            pri_cname, r.n[OCOUNT], r.n[OVNUM],
         ...
         pager_printf(ch,
            "%-12s%3d %5d %-12s %2s %2s%3d%3d%3d%4d%4d%3d"
            "%3d%3d%3d%3d\n\r", 
            pri_cname, r.n[OCOUNT], r.n[OVNUM],
            pri_oname, go_otype_to_disp( r.n[OTYPE] ),
            owear_to_disp( r.n[OWEAR] ),
                    ...
                    bool nm_sw, bool ng_sw )
{
  ...
  {
     if ( get_trust(ch) < GetMaxLevel(po->carried_by) ) return FALSE;
  ...
  else if ( po->in_obj )                 /* it's in a container          */
  {
     ...
     while( pt->in_obj )
            pt=pt->in_obj;
     if ( pt->carried_by && get_trust(ch) < GetMaxLevel(pt->carried_by) )
        return FALSE;
...
bool gr_eval_and (GR_STRUCT r, int op_num)
{
  ...
  for (cou=0; cou<op_num; cou++)
  {
     ...
     case ch_class:
        if ( !go_eval_num (r.ch_class, gr_op[cou].op, gr_op[cou].nval) )
           return FALSE;
        else break;
     case race:
     ...
     CHAR_DATA *ch, int op_num, bool or_sw, int dis_num)
{
  ...
  char ch_class[] = "MCTWVDRAPN";              /* convert ch_class to text     */
  ...
  "%-12s %2hd %c %2s %c %3s %3s %5hd %11ld %-15s %6lu %c\n\r", 
              ...
              race[(unsigned char) r.race], ch_class[(unsigned char) r.ch_class],
...
send_to_char( "diagnose mrc num race class vnum1 vnum2 - mobs/race/ch_class\n\r"
   "   display all mobs of a particular race/ch_class combo.\n\r"
...
if (!str_cmp(arg1, "of")) {
   ...
   for (cou = 0; cou < num && freq[cou]; cou++)
       ch_printf(ch, "%3d%8d%8d\n\r", cou+1,freq[cou]->ivnum,freq[cou]->count);
   DISPOSE(freq);
   return;
   }
          ...
          for (pObj=obj_index_hash[cou]; pObj; pObj=pObj->next)
              if (pObj->weight == 0) {
                 ...
                 if (zero_obj_ind <= ZERO_MAX) {
                    vnums[zero_obj_ind - 1] = pObj->ivnum;
...
for (po=first_object; po; po=po->next)
{
    ...
    if ( ch==po->carried_by || ch==pt->carried_by )
    {
       ...
       px->ivnum, px->name, px->extra_flags);
      ...
      ch_printf(ch,
      "\n\rOBJECT_DATA name=%s wear_flags=%d wear_loc=%d\n\r",
      po->name, po->wear_flags, po->wear_loc);
...
{
   MOB_INDEX_DATA *pm;
   int cou, race, ch_class, dis_num, vnum1, vnum2, dis_cou = 0;
   ...
   vnum2    = atoi (arg6);
/*
   ch_printf(ch, "dis_num=%d race=%d ch_class=%d vnum1=%d vnum2=%d\n\r",
       dis_num, race, ch_class, vnum1, vnum2);
*/
   send_to_char("\n\r", ch);
   ...
   for (cou = 0; cou < MAX_KEY_HASH; cou++)
   {
      if ( mob_index_hash[cou] )
         ...
         {
            if ( pm->ivnum >= vnum1 && pm->ivnum <= vnum2
            &&   pm->race==race && IS_ACTIVE(pm, ch_class) && dis_cou++ < dis_num )
                pager_printf( ch, "%5d %s\n\r", pm->ivnum, pm->player_name );
         }
   }
   return;
}
     ...
     for (pObj=obj_index_hash[hash]; pObj; pObj=pObj->next)
         if (pObj->layers > 0)
         {
            ...
            pager_printf(ch, "%4d %5d %9d %5d   %s\n\r",
              cou, pObj->ivnum, pObj->wear_flags, pObj->layers,
              pObj->short_descr);
          }
}
