...
/*static char rcsid[] = "$Id: mud_prog.c,v 1.45 2003/12/21 17:20:55 dotd Exp $";*/
...
#define IFIGNORED    8
#define ORIGNORED    9
#define RETURNTRUE   10
#define RETURNFALSE  11*/
typedef enum
{
    COMMANDOK = 1, IFTRUE, IFFALSE, ORTRUE, ORFALSE, FOUNDELSE,
    FOUNDENDIF, IFIGNORED, ORIGNORED, RETURNTRUE, RETURNFALSE,
    CMDBERR
} prog_cmd_ret;

/* Ifstate defines, used to create and access ifstate array
 in mprog_driver. */
#define MAX_IFS     50		/* should always be generous */
#define IN_IF        0
#define IN_ELSE      1
#define DO_IF        2
#define DO_ELSE      3
...
sh_int mprog_do_command( char *cmnd, CHAR_DATA *mob, CHAR_DATA *actor,
...
void oprog_wordlist_check( char *arg, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, prog_types type, OBJ_DATA *iobj );
void set_supermob(OBJ_DATA *obj);
bool oprog_percent_check( CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, prog_types type);
void rprog_percent_check( CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, prog_types type);
bool rprog_wordlist_check( char *arg, CHAR_DATA *mob, CHAR_DATA *actor,
                           OBJ_DATA *obj, void *vo, prog_types type, ROOM_INDEX_DATA *room );
                      ...
                      OBJ_DATA *obj, void *vo, CHAR_DATA *rndm )
{
    ...
    char rcvar[MAX_INPUT_LENGTH];
    char *point = ifcheck;
    char *pchck = chck;
    CHAR_DATA *chkchar = NULL, *rchkchar = NULL;
    OBJ_DATA *chkobj = NULL, *rchkobj = NULL;
    int lhsvl, rhsvl;
    cvar[0] = '\0';
    rcvar[0] = '\0';
    ...
    *pchck = '\0';
     ...
     * operator, rcvar is variable in () of rval
     */
    if ( cvar[0] == '$' )
    {
        switch(cvar[1])
...
/*                sprintf(rval, "$r without rndm");
    ...
    if (rval[0] == '%' || rval[0] == '#')
    {
        ...
        if (rchkchar)
            val = get_var_val_ch(rchkchar, &rval[1]);
        else
            val = get_var_val_obj(rchkobj, &rval[1]);
        if (val)
            strcpy(rval, val);
        else
            rval[0] = '\0';
    ...
    if (chck[0] == '%')
    {
        ...
        if (chkchar)
            val = get_var_val_ch(chkchar, &chck[1]);
        else
            val = get_var_val_obj(chkobj, &chck[1]);
        if (!val)
            val = "";

        return mprog_seval(val, opr, rval, mob);
    }
    if (chck[0] == '#')
    {
        if (chkchar)
            lhsvl = get_var_val_int_ch(chkchar, &chck[1]);
        else
            lhsvl = get_var_val_int_obj(chkobj, &chck[1]);
    ...
    if ( !str_cmp(chck, "varexists") )
    {
        if (!chkchar && !chkobj)
            return FALSE;
        return (get_var(chkchar?chkchar->vars:chkobj->vars, rval)?TRUE:FALSE);
    }
    ...
    if ( !str_cmp(chck, "numprcnt") )
    {
        return mprog_veval(number_percent(), opr, atoi(rval), mob);
    }
    if ( !str_cmp(chck, "economy") )
    {
        ...
        return mprog_veval( ((room->area->high_economy[DEFAULT_CURR] > 0) ? 1000000000 : 0)
                            + room->area->low_economy[DEFAULT_CURR], opr, atoi(rval), mob );
    }
    ...
    if ( !str_cmp(chck, "master") )
    {
        ...
        if ( vnum < 1 || vnum > 100000 )
        {
            progbug( "Bad vnum to 'master'", mob );
            return CMDBERR;
        }
        lhsvl = 0;
    ...
    if ( !str_cmp(chck, "oexists") )
    {
       OBJ_INDEX_DATA *pObjIndx;
       int vnum = atoi(cvar);

       if (!obj_exists_index(vnum))
       {
          progbug("Oexists: bad vnum", mob);
          return CMDBERR;
       }

       if (!is_number(rval))
       {
          progbug("Oexists: bad expression", mob);
          return CMDBERR;
       }

       pObjIndx = get_obj_index(vnum);

       return mprog_veval(pObjIndx->count, opr, atoi(rval), mob);
    }
    if ( !str_cmp(chck, "mexists") )
    {
       MOB_INDEX_DATA *pMobIndx;
       int vnum = atoi(cvar);

       if (!obj_exists_index(vnum))
       {
          progbug("Mexists: bad vnum", mob);
          return CMDBERR;
       }

       if (!is_number(rval))
       {
          progbug("Mexists: bad expression", mob);
          return CMDBERR;
       }

       pMobIndx = get_mob_index(vnum);

       return mprog_veval(pMobIndx->count, opr, atoi(rval), mob);
    }
    if ( !str_cmp(chck, "ovnumhere") )
    {
        ...
        for ( pObj = mob->first_carrying; pObj; pObj = pObj->next_content )
            if ( can_see_obj(mob, pObj) && pObj->vnum == vnum )
                lhsvl++;
    ...
    if ( !str_cmp(chck, "iswearing") ) /* if iswearing($n) in (vnum) */
    {
        OBJ_DATA *pObj;
        int vnum = atoi(rval);
        ...
        if ( str_cmp(opr, "==") )
        {
            progbug("Iswearing: bad operator", mob);
            return CMDBERR;
        }
    ...
    if ( chkchar )
    {
        if ( !str_cmp(chck, "recallpoint" ) )
        {
            return mprog_veval(chkchar->pcdata->home, opr, atoi(rval), mob);
        }
        ...
        if ( !str_cmp(chck, "isgoodrace") )
        {
            return IsGoodSide(chkchar) ? TRUE : FALSE;
        }
        if ( !str_cmp(chck, "isneutral") )
        {
            return IS_NEUTRAL(chkchar) ? TRUE : FALSE;
        }
        if ( !str_cmp(chck, "isneutralrace") )
        {
            return IsNeutralSide(chkchar) ? TRUE : FALSE;
        }
        if ( !str_cmp(chck, "isevil") )
        {
            return IS_EVIL(chkchar) ? TRUE : FALSE;
        }
        if ( !str_cmp(chck, "isevilrace") )
        {
            return IsBadSide(chkchar) ? TRUE : FALSE;
        }
        if ( !str_cmp(chck, "isundead") )
        {
            return IsUndead(chkchar) ? TRUE : FALSE;
        }
        ...
        {
            return mprog_veval(100*GET_HIT(chkchar)/GET_MAX_HIT(chkchar), opr, atoi(rval), mob);
        }
        ...
        {
            return mprog_veval(GET_MONEY(chkchar,DEFAULT_CURR), opr, atoi(rval), mob);
        }
        if ( !str_cmp(chck, "class") )
        {
            return mprog_seval(GetClassString(chkchar), opr, rval, mob);
        }
        if ( !str_cmp(chck, "race") )
        {
            return mprog_seval(race_table[GET_RACE(chkchar)].race_name, opr, rval, mob);
        }
        ...
        {
            return mprog_veval(GET_COND(chkchar, COND_DRUNK), opr, atoi(rval), mob);
        }
    }
    if ( chkobj )
    {
    ...
    if ( !str_cmp(chck, "number") )
    {
        if ( chkchar )
        {
            if ( !IS_NPC(chkchar) )
                return FALSE;
            lhsvl = (chkchar == mob) ? GET_MONEY(chkchar,DEFAULT_CURR) : chkchar->vnum;
            return mprog_veval(lhsvl, opr, atoi(rval), mob);
        }
        return mprog_veval(chkobj->vnum, opr, atoi(rval), mob);
    }
...
/*    if IS_AFFECTED( mob, AFF_CHARM )
    ...
    while ( TRUE )
    {
        ...
        case COMMANDOK:
#ifdef DEBUG
            log_string_plus( "COMMANDOK", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case IFTRUE:
#ifdef DEBUG
            log_string_plus( "IFTRUE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case IFFALSE:
#ifdef DEBUG
            log_string_plus( "IFFALSE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case ORTRUE:
#ifdef DEBUG
            log_string_plus( "ORTRUE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case ORFALSE:
#ifdef DEBUG
            log_string_plus( "ORFALSE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case FOUNDELSE:
#ifdef DEBUG
            log_string_plus( "FOUNDELSE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case FOUNDENDIF:
#ifdef DEBUG
            log_string_plus( "FOUNDENDIF", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
        ...
        case IFIGNORED:
#ifdef DEBUG
            log_string_plus( "IFIGNORED", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
            if ( !( ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE] ) )
            {
        ...
        case ORIGNORED:
#ifdef DEBUG
            log_string_plus( "ORIGNORED", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
            if ( !( ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE] ) )
            {
                progbug( "Unmatched or", mob );
        ...
        case CMDBERR:
#ifdef DEBUG
            log_string_plus( "CMDBERR", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
            --prog_nest;
            return FALSE;
            break;
        ...
        case RETURNTRUE:
#ifdef DEBUG
            log_string_plus( "RETURNTRUE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
            --prog_nest;
            return TRUE;
            break;
        ...
        case RETURNFALSE:
#ifdef DEBUG
            log_string_plus( "RETURNFALSE", LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
            --prog_nest;
            return FALSE;
            break;
			...
			bool ignore, bool ignore_ors )
{
    ...
    if ( !str_cmp( firstword, "return" ) )
        return atoi(ifcheck)>0?RETURNTRUE:RETURNFALSE;
...
bool mprog_wordlist_check( char *arg, CHAR_DATA *mob, CHAR_DATA *actor,
                           OBJ_DATA *obj, void *vo, sh_int type )
{
                ...
                while ( ( start = strstr( dupl, list ) ) )
                    if ( (start == dupl || *(start-1) == ' ' )
                         && ( *(end = start + strlen( list ) ) == ' '
                              ...
                              || *end == '\0' ) )
                    {
                        return mprog_driver( mprg->comlist, mob, actor, obj, vo, FALSE );
                    }
                    else
                        dupl = start+1;
            }
                       ...
                       void *vo, prog_types type)
{
    ...
    for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
    {
        ...
        if ( ( IS_PROG_TYPE(mprg, type) )
             && ( ( !mprg->triggered ) || ( IS_PROG_TYPE(mprg, HOUR_PROG) ) ) )
        {
            mprg->triggered = TRUE;
            mprog_driver( mprg->comlist, mob, actor, obj, vo, FALSE );
        }
...
void mprog_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
         ...
         && HAS_PROG(mob->pIndexData, BRIBE_PROG) )
    {
        ...
        snprintf(buf, 127, "bribe_prog %d %s", ch->vnum, ch->name);
        obj->last_carried_by = str_dup(buf);
        obj = obj_to_char( obj, mob );

        for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
            if ( ( IS_PROG_TYPE(mprg, BRIBE_PROG) )
...
void mprog_birth_trigger( CHAR_DATA *maker, CHAR_DATA *mob )
{
    if ( IS_NPC( mob )
         && HAS_PROG(mob->pIndexData, BIRTH_PROG) )
    {
        mprog_percent_check( mob,maker, NULL, NULL, BIRTH_PROG );
    }
    return;
}
...
void mprog_give_trigger( CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
    {
        if (!IS_PROG_TYPE(mprg, GIVE_PROG))
            continue;
        ...
        if ( !str_cmp( obj->name, mprg->arglist ) ||
             !str_cmp( "all", buf ) ||
             ( obj->pIndexData && obj->vnum == atoi(buf) ) )
        {
...
void mprog_greet_trigger( CHAR_DATA *ch )
{
    ...
    log_string_plus( buf, LOG_DEBUG, LEVEL_LOG_CSET, SEV_INFO );
#endif
...
void mprog_hitprcnt_trigger( CHAR_DATA *mob, CHAR_DATA *ch)
{
    ...
    if ( IS_NPC( mob )
        ...
        for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
            if ( ( IS_PROG_TYPE(mprg, HITPRCNT_PROG) )
...
void mprog_leave_trigger( CHAR_DATA *actor )
{
    CHAR_DATA *vmob;

    for ( vmob = actor->in_room->first_person; vmob; vmob = vmob->next_in_room )
    {
        if ( IS_NPC( vmob ) && HAS_PROG(vmob->pIndexData, LEAVE_PROG) )
        {
            if ( IS_NPC( actor ) && actor->pIndexData == vmob->pIndexData )
                continue;
            mprog_percent_check(vmob,actor,NULL,NULL,LEAVE_PROG);
        }
    }
}
...
void mprog_script_trigger( CHAR_DATA *mob )
{
    ...
    if ( HAS_PROG(mob->pIndexData, SCRIPT_PROG) )
        for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next )
            if ( IS_PROG_TYPE(mprg, SCRIPT_PROG) )
            {
                if ( mprg->arglist[0] == '\0'
                     ||   mob->mpscriptpos != 0
...
void set_supermob( OBJ_DATA *obj)
{
    ...
    if ( in_obj->carried_by )
    {
        ...
        /* so mposet works, kinda scary -Garil 10/16/2002 */
        supermob->first_carrying = in_obj->carried_by->first_carrying;
        supermob->last_carrying = in_obj->carried_by->last_carrying;
    }
    else
    {
        room = obj->in_room;
...
{
    supermob->first_carrying = NULL;
    supermob->last_carrying = NULL;
    char_from_room( supermob );
...
void oprog_wordlist_check( char *arg, CHAR_DATA *mob, CHAR_DATA *actor,
                           OBJ_DATA *obj, void *vo, prog_types type, OBJ_DATA *iobj )
{
...
{
    if ( HAS_PROG(ch->in_room, LOOK_PROG) )
    {
        /* supermob is set and released in rprog_wordlist_check */
        rprog_wordlist_check( txt, supermob, ch, NULL, NULL, LOOK_PROG, ch->in_room );
    }
    return;
}
                           ...
                           OBJ_DATA *obj, void *vo, prog_types type, ROOM_INDEX_DATA *room )
{
    ...
    for ( mprg = room->mudprogs; mprg; mprg = mprg->next )
        if ( IS_PROG_TYPE(mprg, type) )
        {
            if ((!arg || !*arg) && (!mprg->arglist || !*mprg->arglist))
            {
                rset_supermob( room );
                mprog_driver( mprg->comlist, mob, actor, obj, vo, FALSE );
                release_supermob();
...
void progbug( char *str, CHAR_DATA *mob )
{
    ...
    if ( mob->vnum == 3 )
    {
        ...
        log_printf_plus(LOG_BUILD, LEVEL_IMMORTAL, SEV_DEBUG,
                        "%s, %s.", str,
                        mob->description?mob->description:"(unknown)" );
        return;
    }

    log_printf_plus(LOG_BUILD, LEVEL_IMMORTAL, SEV_DEBUG,
                    "%s, Mob v%d, u%d.", str, mob->vnum, mob->unum );
}
...
{
    if ( HAS_PROG(mob->in_room, QUEST_PROG) )
    {
        rset_supermob( mob->in_room );
        rprog_percent_check(supermob, mob, NULL, NULL, QUEST_PROG);
        release_supermob();
    }
}
...
{
    if ( IS_NPC(mob) && HAS_PROG(mob->pIndexData, QUEST_PROG) )
        mprog_percent_check( mob, ch, NULL, NULL, QUEST_PROG );
}

void oprog_quest_trigger( CHAR_DATA *ch, OBJ_DATA *obj )
{
    if ( HAS_PROG(obj->pIndexData, QUEST_PROG) )
    {
        set_supermob( obj );
        oprog_percent_check( supermob, ch, obj, NULL, QUEST_PROG );
        release_supermob();
    }
}
...
bool mprog_command_trigger( char *txt, CHAR_DATA *mob, CHAR_DATA *actor )
{
    CHAR_DATA *vmob;

    for ( vmob = actor->in_room->first_person; vmob; vmob = vmob->next_in_room )
    {
        if ( IS_NPC( vmob ) && HAS_PROG(vmob->pIndexData, COMMAND_PROG) )
        {
            if ( IS_NPC( actor ) && actor->pIndexData == vmob->pIndexData )
                continue;
            if ( mprog_wordlist_check( txt, vmob, actor, NULL, NULL, COMMAND_PROG ) )
                return TRUE;
        }
    }
    return FALSE;
...
bool rprog_command_trigger( char *txt, CHAR_DATA *ch )
{
    ...
    {
        if ( rprog_wordlist_check( txt, supermob, ch, NULL, NULL, COMMAND_PROG, ch->in_room ) )
            return TRUE;
    }
...
void rprog_area_reset_trigger( ROOM_INDEX_DATA *room )
{
    ...
    if ( !HAS_PROG(room, AREA_RESET_PROG) )
        return;

    for ( mprg = room->mudprogs; mprg; mprg = mprg->next )
        if ( IS_PROG_TYPE(mprg, AREA_RESET_PROG) )
        {
            rset_supermob( room );
            mprog_driver( mprg->comlist, supermob, NULL, NULL, NULL, TRUE );
            release_supermob();
...
void rprog_area_init_trigger( ROOM_INDEX_DATA *room )
{
    ...
    if ( !HAS_PROG(room, AREA_INIT_PROG) )
        return;

    for ( mprg = room->mudprogs; mprg; mprg = mprg->next )
        if ( IS_PROG_TYPE(mprg, AREA_INIT_PROG) )
        {
            rset_supermob( room );
            mprog_driver( mprg->comlist, supermob, NULL, NULL, NULL, TRUE );
            release_supermob();
    ...
    for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
        for ( pRoomIndex = room_index_hash[hash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
            if (pRoomIndex->area == area)
                rprog_area_init_trigger(pRoomIndex);
}
...
void aprog_reset_trigger( AREA_DATA *area )
{
    ...
    for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
        for ( pRoomIndex = room_index_hash[hash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
            if (pRoomIndex->area == area)
                rprog_area_reset_trigger(pRoomIndex);
}
