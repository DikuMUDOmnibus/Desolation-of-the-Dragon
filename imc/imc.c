...
char *generate2( PACKET *p )
{
   ...
   if( !p->i.path[0] )
      imcstrlcpy( newpath, imc_siteinfo.localname, IMC_PATH_LENGTH );
   else
      snprintf( newpath, IMC_PATH_LENGTH, "%s!%s", p->i.path, imc_siteinfo.localname );
...
char *getname( CHAR_DATA *ch, imc_char_data *vict )
{
   ...
   if( !strcasecmp( mud, imc_siteinfo.localname ) )
...
void imc_delete_info( void )
{
   ...
   IMCSTRFREE( imc_siteinfo.localname );
   IMCSTRFREE( imc_siteinfo.fullname );
...
void imc_send( PACKET *p )
{
   ...
   if( !strcasecmp( p->type, "remote-admin" ) && this_imcmud->md5pass == TRUE )
   {
      ...
      snprintf( salt, 10, "$1$%s$", imc_mudof( p->to ) );
      cryptpwd = crypt( pwd, salt );
      imc_addkey( p, "hash", cryptpwd );
   }

   imcstrlcpy( p->i.to, p->to, IMC_NAME_LENGTH );
   snprintf( p->i.from, IMC_NAME_LENGTH, "%s@%s", p->from, imc_siteinfo.localname );
   do_send_packet( p );
}
...
void updateroutes( char *path )
{
   ...
   while( temp && temp[0] != '\0' )
   {
      ...
      if( strcasecmp( sender, imc_siteinfo.localname ) )
      {
         /* not from us */
...
void imc_showchannel( IMC_CHANNEL *c, char *from, char *txt, int emote )
{
   ...
   for( d = first_descriptor; d; d = d->next )
   {
	if( !d->character )
      ...
      if( !c->open )
      {
         snprintf( name, SMST, "%s@%s", CH_IMCNAME(ch), imc_siteinfo.localname );
...
void process_rwho( imc_char_data *from, char *argument )
{
   ...
   snprintf( whobuf, LGST, "~R-=[ ~WPlayers on %s ~R]=-", imc_siteinfo.fullname );
   imcstrlcpy( buf, imccenterline( whobuf, 78 ), LGST );
   imcstrlcat( buf, "\n\r", LGST );
   imc_whoreply_add( buf );
    
   if( imc_siteinfo.port > 0 )
      snprintf( whobuf, LGST, "~Y-=[ ~Wtelnet://%s:%d ~Y]=-", imc_siteinfo.host, imc_siteinfo.port );
   else
      snprintf( whobuf, LGST, "~Y-=[ telnet://%s ]=-", imc_siteinfo.host );
   ...
   snprintf( tailbuf, LGST, "~Y[~WHomepage: %s~Y] [~W%3d Player%s Online~Y]\n\r",
      imc_siteinfo.www, xx+yy, ( xx+yy == 1 ) ? "" : "s" );
   imc_whoreply_add( tailbuf );
...
void imc_recv_who( imc_char_data *from, char *type )
{
   ...
   else if( !strcasecmp( arg, "finger" ) )
   {
      ...
      snprintf( output, LGST,
         ...
         "~cIMC Version    ~W: ~!%s\n\r"
         "~cIMC Details    ~W: ~!%s\n\r",
         imc_siteinfo.fullname, imc_siteinfo.host, imc_siteinfo.email, imc_siteinfo.www, IMC_VERSIONID, imc_siteinfo.details );
   else
...
void imc_recv_keepalive( char *from, char *version, char *flags, char *network )
{
   ...
   if( !strcasecmp( from, imc_siteinfo.localname ) )
      return;
...
void imc_send_keepalive( const char *to )
{
   ...
   imc_addkey( &out, "url", imc_siteinfo.www );
...
void imc_recv_channel_who( char *from, char *channel, char *lname, int level )
{
   IMC_CHANNEL *c;
   DESCRIPTOR_DATA *d;
   CHAR_DATA *person;
...
void imc_recv( PACKET *p )
{
   ...
   {
      imc_recv_channel_who( p->from, imc_getkey( p, "channel", "" ),
...
void do_imcread( void )
{
   ...
   {
      if( r < 0 ) /* read error */
         imclog( "Read error on connection to %s", this_imcmud->routername );
      else        /* socket was closed */
         imclog( "EOF encountered on connection to %s", this_imcmud->routername );
...
void do_imcwrite( void )
{
   ...
   {
      if( w < 0 )			/* write error */
         imcbug( "Write error on connection to %s", this_imcmud->routername );
      else			/* socket was closed */
         imcbug( "EOF encountered on connection to %s", this_imcmud->routername );
...
void imc_handle_autosetup( char *source, char *routername, char *cmd, char *txt, char *md5 )
{
   if( !strcasecmp( cmd, "reject" ) )
   {
      ...
      {
         imclog( "There is already a mud named %s connected to the network.", imc_siteinfo.localname );
         imc_shutdown( FALSE );
         return;
      }
...
void serverpassword( char *argument )
{
   ...
   if( !strcasecmp( arg1, "MD5-AUTH-INIT" ) )
   {
      ...
      snprintf( salt, 10, "$1$%s$", imc_siteinfo.localname );
      cryptpwd = crypt( pwd, salt );

      snprintf( response, IMC_PACKET_LENGTH, "MD5-AUTH-RESP %s %s version=%d",
         imc_siteinfo.localname, cryptpwd, IMC_VERSION );
      do_imcsend( response );
      return;
   }
         ...
         case IMC_CONNECTED:
	      ...
	      if( p )
	      {
	         ...
	         imc_stats.rx_pkts++;
               ...
               route = imc_find_reminfo( imc_mudof( p->i.from ) );
               if( route )
               {
               ...
               if( !strcasecmp( imc_mudof( p->i.to ), "*" ) || !strcasecmp( imc_mudof( p->i.to ), imc_siteinfo.localname ) )
               {
...
void imc_adjust_perms( CHAR_DATA *ch )
{
   ...
   {
      if( CH_IMCLEVEL(ch) < imc_siteinfo.minlevel )
	   IMCPERM(ch) = IMCPERM_NONE;
      else if( CH_IMCLEVEL(ch) >= imc_siteinfo.minlevel && CH_IMCLEVEL(ch) < imc_siteinfo.immlevel )
	   IMCPERM(ch) = IMCPERM_MORT;
      else if( CH_IMCLEVEL(ch) >= imc_siteinfo.immlevel && CH_IMCLEVEL(ch) < imc_siteinfo.adminlevel )
	   IMCPERM(ch) = IMCPERM_IMM;
      else if( CH_IMCLEVEL(ch) >= imc_siteinfo.adminlevel && CH_IMCLEVEL(ch) < imc_siteinfo.implevel )
	   IMCPERM(ch) = IMCPERM_ADMIN;
      else if( CH_IMCLEVEL(ch) >= imc_siteinfo.implevel )
...
void imc_char_login( CHAR_DATA *ch )
{
   ...
   snprintf( buf, IMC_NAME_LENGTH, "%s@%s", CH_IMCNAME(ch), imc_siteinfo.localname );
...
void imc_freechardata( CHAR_DATA *ch )
{
   ...
   {
      imclog( "IMC2 data for %s has not been initialized yet!", CH_IMCNAME(ch) );
      return;
   }
	      ...
	      if ( !strcasecmp( word, "End" ) )
		   ...
		   if( channel->level > IMCPERM_IMP )
		   {
			/* The IMCPERM_NONE condition should realistically never happen.... */
      		if( channel->level < imc_siteinfo.minlevel )
	   		   channel->level = IMCPERM_NONE;
      		else if( channel->level >= imc_siteinfo.minlevel && channel->level < imc_siteinfo.immlevel )
	   		   channel->level = IMCPERM_MORT;
      		else if( channel->level >= imc_siteinfo.immlevel && channel->level < imc_siteinfo.adminlevel )
	   		   channel->level = IMCPERM_IMM;
      		else if( channel->level >= imc_siteinfo.adminlevel && channel->level < imc_siteinfo.implevel )
	   		   channel->level = IMCPERM_ADMIN;
      		else if( channel->level >= imc_siteinfo.implevel )
   ...
   for( ;; )
   {
	...
	switch( word[0] ) 
	{
	   ...
	   case 'A':
		...
		IMCKEY( "AdminLevel",	imc_siteinfo.adminlevel,	imcfread_number( fin ) );
		break;
	   ...
	   case 'E':
		...
		break;
         ...
         case 'H':
		IMCKEY( "HubAddr",	this_imcmud->host,		imcfread_line( fin ) );
		IMCKEY( "HubPort",	this_imcmud->port,		imcfread_number( fin ) );
            break;

	   case 'I':
            IMCKEY( "Implevel",     imc_siteinfo.implevel,  imcfread_number( fin ) );
		IMCKEY( "InfoName",	imc_siteinfo.fullname,	imcfread_line( fin ) );
		IMCKEY( "InfoHost",	imc_siteinfo.host,	imcfread_line( fin ) );
		IMCKEY( "InfoPort",	imc_siteinfo.port,	imcfread_number( fin ) );
		IMCKEY( "InfoEmail",	imc_siteinfo.email,	imcfread_line( fin ) );
		IMCKEY( "InfoWWW",	imc_siteinfo.www,		imcfread_line( fin ) );
		IMCKEY( "InfoDetails",	imc_siteinfo.details,	imcfread_line( fin ) );
		IMCKEY( "InfoBase",	imc_siteinfo.base,	imcfread_line( fin ) );
		break;

	   case 'L':
		IMCKEY( "LocalName",       imc_siteinfo.localname,    imcfread_line( fin ) );
		break;

	   case 'M':
            IMCKEY( "MD5",             this_imcmud->md5,          imcfread_number( fin ) );
            IMCKEY( "MD5Pwd",          this_imcmud->md5pass,      imcfread_number( fin ) );
            IMCKEY( "MinImmLevel",     imc_siteinfo.immlevel,     imcfread_number( fin ) );
            IMCKEY( "MinPlayerLevel",  imc_siteinfo.minlevel,     imcfread_number( fin ) );
		break;

         case 'R':
            IMCKEY( "RouterAddr",   this_imcmud->host,            imcfread_line( fin ) );
            IMCKEY( "RouterPort",   this_imcmud->port,            imcfread_number( fin ) );
            break;
...
bool imc_read_config( void ) 
{
   ...
   for( ; ; )
   {
	...
	{
	   IMCCREATE( this_imcmud, ROUTERINFO, 1 );

         /* If someone can think of better default values, I'm all ears. Until then, keep your bitching to yourselves. */
	   imc_siteinfo.minlevel = 10;
         imc_siteinfo.immlevel = 101;
	   imc_siteinfo.adminlevel = 113;
         imc_siteinfo.implevel = 115;
   ...
   if( !this_imcmud->host || !this_imcmud->clientpw || !this_imcmud->serverpw )
   {
   ...
   if( !imc_siteinfo.localname || imc_siteinfo.localname[0] == '\0' )
   {
   ...
   if( !imc_siteinfo.fullname || imc_siteinfo.fullname[0] == '\0' )
   {
...
bool imc_connect_to( void )
{
    ...
    */
   if( !inet_aton( this_imcmud->host, &sa.sin_addr ) )
   {
	hostp = gethostbyname( this_imcmud->host );
	if( !hostp )
      {
   ...
   if( this_imcmud->md5 )
   {
      ...
      if( !this_imcmud->md5pass )
      {
         snprintf( buf, IMC_DATA_LENGTH, "PW %s %s version=%d autosetup %s MD5",
            imc_siteinfo.localname, this_imcmud->clientpw, IMC_VERSION, this_imcmud->serverpw );
      }
       ...
       */
         ...
         snprintf( buf, IMC_PACKET_LENGTH, "MD5-AUTH-REQ %s", imc_siteinfo.localname );
   }
...
void imc_startup( bool force )
{
   ...
   imc_active = imc_siteinfo.localname ? IA_CONFIG2 : IA_CONFIG1;
...
IMC_CMD( imclist )
{
   ...
   imcpager_printf( ch, "\n\r\n\r~c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
      imc_siteinfo.localname, IMC_VERSIONID, this_imcmud->network, this_imcmud->routername );
...
IMC_CMD( imcconfig )
{
   ...
   {
      IMCSTRFREE( imc_siteinfo.localname );
      imc_siteinfo.localname = IMCSTRALLOC( argument );
      imc_save_config();
   ...
   {
      IMCSTRFREE( imc_siteinfo.fullname );
      imc_siteinfo.fullname = IMCSTRALLOC( argument );
	imc_save_config();
...
IMC_CMD( imcchanwho )
{
   ...
   argument = one_argument( argument, chan );
...
char *imc_find_social( CHAR_DATA *ch, char *sname, char *person, char *mud, int victim )
{
   ...
   if( person && person[0] != '\0' && mud && mud[0] != '\0' )
   {
	if( person && person[0] != '\0' && !strcasecmp( person, CH_IMCNAME(ch) ) 
	 && mud && mud[0] != '\0' && !strcasecmp( mud, imc_siteinfo.localname ) )
	{
...
char *imc_act_string( const char *format, CHAR_DATA *ch, CHAR_DATA *vic )
{
   ...
   while( *format != '\0' )
   {
      ...
      else
      {
         ...
         {
            default:  i = " !!!!! "; break;
            case 'n': i = imc_makename( CH_IMCNAME(ch), imc_siteinfo.localname, FALSE ); break;
            ...
            case 'K': one_argument( CH_IMCNAME(vic), tmp_str ); i = (char *)tmp_str; break;
         }
      }
      ++format;
      while( ( *point = *i ) != '\0' )
